    ...
    return (
      <DropdownMenu open={isOpen} onOpenChange={toggleDropdown}>
        <DropdownMenuTrigger
          ...
          className={cn('relative flex cursor-pointer items-center justify-between gap-2 rounded-md text-base font-semibold', className)}
        >
        ...
        >
          {candidates.map((candidate, index) => (
            <DropdownMenuItem key={index} onClick={() => setvalue(candidate)} className='relative z-10 cursor-pointer px-3 py-2 font-medium hover:bg-swBlack'>
              {candidate}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
